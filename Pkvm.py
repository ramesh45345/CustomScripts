...
def ovmf_bin_nvramcopy(destpath: str, vmname: str, secureboot: bool = False):
    ...
    ovmf_bin_options = [os.path.join(destpath, "OVMF_CODE.fd"),
                                   "/usr/share/OVMF/OVMF_CODE.fd",
                                   "/usr/share/ovmf/x64/OVMF_CODE.fd",
                                   "/usr/share/OVMF/OVMF_CODE_4M.fd",
                                   "/bin/OVMF_CODE.fd"]
    ovmf_vars_options = [os.path.join(destpath, "OVMF_VARS.fd"),
                                    "/usr/share/OVMF/OVMF_VARS.fd",
                                    "/usr/share/ovmf/x64/OVMF_VARS.fd",
                                    "/usr/share/OVMF/OVMF_VARS_4M.fd",
                                    "/bin/OVMF_VARS.fd"]
    ovmf_bin_secboot_options = ["/usr/share/OVMF/OVMF_CODE.secboot.fd",
                                           "/usr/share/ovmf/x64/OVMF_CODE.secboot.fd",
                                           "/usr/share/OVMF/OVMF_CODE_4M.secboot.fd",
                                           "/bin/OVMF_CODE.secboot.fd"] + ovmf_bin_options
    ovmf_vars_secboot_options = ["/usr/share/OVMF/OVMF_VARS.secboot.fd",
                                            "/usr/share/OVMF/OVMF_VARS_4M.ms.fd",
                                            "/bin/OVMF_VARS.secboot.fd"] + ovmf_vars_options
    # Search for efi bin
...
if __name__ == '__main__':
    ...
    if 45 <= args.ostype <= 49:
        ...
        # Detect Alpine URL
        import CAlpineUrl
        isourl = CAlpineUrl.find_latest_standard_x86_64_iso(CAlpineUrl.fetch_html())
    if args.ostype == 45:
        vmname = "AlpineVM"
        if args.desktopenv is None:
            args.desktopenv = "kde"
    ...
    if 50 <= args.ostype <= 59:
        ...
        data['build']['provisioner'][2]["powershell"]["inline"] = [
            r'''powershell -executionpolicy bypass "& Set-Variable ProgressPreference SilentlyContinue; &'{0}'; &'C:/Python*/python.exe' {1}; &'C:/Python*/python.exe' {2}; Remove-Item -Recurse {3} ; exit 0;"'''.format(os.path.join("C:/", tempscriptbasename, "Win-provision.ps1"), os.path.join("C:/", tempscriptbasename, "Wwinget.py"), os.path.join("C:/", tempscriptbasename, "Wwinget.py"), os.path.join("C:/", tempscriptbasename)),
        ]
